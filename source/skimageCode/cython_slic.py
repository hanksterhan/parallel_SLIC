...
cudaSLICstring = SourceModule(
"""
//# This code should be run with one thread per kernal
__global__ void cudaSLIC(float *a) {
    //---------------------------------------------------------------
    //# Find approprate cluster for every pixel
    //# -- saves the cluster name (n) to klabels
    //#TODO: add support for z dimension, try other distance functions

    //#TODO: fill in these variables based on image and command line args
    int img_width = 1000;
    int numk = 100;

    //#TODO: make the following structures
    //# img_l, img_a, img_b: 1D int vectors with length img_width*img_height
    //# distvec: 1D double vector with length img_width*img_height, initialized on each iteration
    //# img_klabels: 1D int vectors with length img_width*img_height
    //# kcent_l, kcent_a, kcent_b, kcent_x, kcent_y: 1D int vectors with length k
    ...
    for(n = 0; n < numk; n++) {
        //# define local region around centroid
        ...
        //# loop over neighbourhood of each centroid
        for(y = y1; y < y2; y++) {
            for(x = x1; x < x2; x++) {
                img_idx = (y * img_width) + x;

                l = img_l[img_idx]; //#TODO: get LAB from correct places
                a = img_a[img_idx];
                b = img_b[img_idx];

                dist_lab = (l - kcent_l[n])*(l - kcent_l[n]) +
                         (a - kcent_a[n])*(a - kcent_a[n]) +
                         (b - kcent_b[n])*(b - kcent_b[n]);

                dist_xy = (x - kcent_x[n])*(x - kcent_x[n]) +
                        (y - kcent_y[n])*(y - kcent_y[n]);

                dist = dist_lab + dist_xy; //#this is approximate, doesnt include sqrts

                if(dist < distvec[img_idx]) {
                  distvec[img_idx] = dist;
                  img_klabels[img_idx] = n;
                }
            }
        }
    }
...
__global__ void cudaSLIC(float**** image_zyx, ) {
    //---------------------------------------------------------------
    //# Find approprate cluster for every pixel
    //# -- saves the cluster name (n) to klabels
    //#TODO: add support for z dimension, try other distance functions
...
def cuda_white_string = SourceModule(
"""
//# This code should be run with one thread per pixel (max img size is 4096x4096)
//# makes whole image white
__global__ void make_white(float* img) {

    // convert from thread+block indices to 1D image index
    int bx, by, bz, tx, ty, tz, tid, bidx, idx;
    bx = blockIdx.x;
    by = blockIdx.y;
    bz = blockIdx.z;
    tx = threadIdx.x;
    ...
    // use 1D img idx to set all pixels to white
    img[3 * idx + 0] = (float) 100; // L
