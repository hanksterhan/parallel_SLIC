	...
	const double&				M)
{
	...
	vector<double> clustersize(numk, 0); //number of pixels in cluster
	...
	for( int itr = 0; itr < 10; itr++ )
	{
      ...
      x2 = min((double)m_width,	kseedsx[n]+offset);
		...
		// accumulates labxy values for each cluster based on the pixels in it
		{
			int ind(0);
		...
		// find average by dividing by number of pixels in cluster
		{
			for( int k = 0; k < numk; k++ )
			{
				if( clustersize[k] <= 0 ) clustersize[k] = 1;
				inv[k] = 1.0/clustersize[k];//computing inverse now to multiply, then divide later
			}}
